...
def test_rminhash_profile_line_reports_kernel_and_pipeline_mode():
    code = """
from rensa import RMinHash
token_sets = [["alpha", "beta"], ["gamma", "delta"]]
RMinHash.digests_from_token_sets(token_sets, num_perm=32, seed=42)
"""
    env = os.environ.copy()
    env["RENSA_PROFILE_BATCH"] = "1"
    proc = subprocess.run(
        [sys.executable, "-c", code],
        text=True,
        capture_output=True,
    ...
    assert "doc_chunk_size=" in proc.stderr
    assert "doc_par_batch_size=" in proc.stderr
    assert "pipeline_queue_cap=" in proc.stderr


def test_rminhash_profile_line_respects_runtime_pipeline_config():
    code = """
from rensa import RMinHash
token_sets = [["alpha", "beta"], ["gamma", "delta"]]
RMinHash.digests_from_token_sets(token_sets, num_perm=32, seed=42)
"""
    env = os.environ.copy()
    env["RENSA_PROFILE_BATCH"] = "1"
    env["RENSA_DOC_CHUNK_SIZE"] = "512"
    env["RENSA_DOC_PAR_BATCH_SIZE"] = "64"
    env["RENSA_PIPELINE_QUEUE_CAP"] = "5"
    proc = subprocess.run(
        [sys.executable, "-c", code],
        text=True,
        capture_output=True,
    ...
    assert "doc_chunk_size=512" in proc.stderr
    assert "doc_par_batch_size=64" in proc.stderr
    assert "pipeline_queue_cap=5" in proc.stderr


def test_rminhash_rho_profile_line_reports_sparse_counters():
    code = """
from rensa import RMinHash
token_sets = [["tok" + str(i % 5) for i in range(4 + (doc_idx % 40))] for doc_idx in range(64)]
RMinHash.digest_matrix_from_token_sets_rho(token_sets, num_perm=128, seed=42, probes=4)
"""
    env = os.environ.copy()
    env["RENSA_PROFILE_BATCH"] = "1"
    proc = subprocess.run(
    ...
    assert "kernel=rho" in proc.stderr
    assert "rho_non_empty_p50=" in proc.stderr
    assert "rho_non_empty_p90=" in proc.stderr
    assert "rho_non_empty_p99=" in proc.stderr
    assert "rho_sparse_row_rate=" in proc.stderr
