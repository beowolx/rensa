...
#[test]
fn seedless_state_is_rejected() {
  Python::initialize();
  let mut lsh = RMinHashLSH::from_validated(0.8, 16, 4, 42);
  lsh.seed = None;

  let error = lsh
    .validate_state()
    .expect_err("seedless LSH state should be rejected");
  assert!(error.to_string().contains("missing seed metadata"));
}

#[test]
fn validated_constructor_preserves_seed() {
  let lsh = RMinHashLSH::from_validated(0.8, 16, 4, 42);

  assert_eq!(lsh.seed().expect("validated LSH should expose a seed"), 42);
}
