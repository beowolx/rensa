...
use pyo3::prelude::*;
use rensa_core::utils::calculate_band_hash;
...
impl RMinHashLSH {
  ...
  pub(in crate::lsh) fn validate_state(&self) -> PyResult<()> {
    ...
    if self.seed.is_none() {
      return Err(PyValueError::new_err(
        "invalid RMinHashLSH state: missing seed metadata, rebuild this index with a seed-aware version of Rensa",
      ));
    }
  ...
  pub(crate) fn seed(&self) -> PyResult<u64> {
    self.seed.ok_or_else(|| {
      PyValueError::new_err(
        "RMinHashLSH is missing seed metadata, rebuild this index with a seed-aware version of Rensa",
      )
    })
  }

  pub(in crate::lsh) fn ensure_seed_compatible(
    &self,
    seed: u64,
  ) -> PyResult<()> {
    ...
    if seed != expected_seed {
      return Err(PyValueError::new_err(format!(
        "seed mismatch: LSH expects {expected_seed}, received {seed}"
      )));
    }
    Ok(())
  }
