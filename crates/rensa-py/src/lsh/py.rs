...
impl RMinHashLSH {
  ...
  /// * `seed` - The seed used to construct all `RMinHash` values inserted into this index.
  ///
  /// # Errors
  ///
  ...
  pub fn new(
    ...
    Ok(Self::from_validated(threshold, num_perm, num_bands, seed))
  }
  ...
  /// # Errors
  ///
  /// Returns an error if the supplied `MinHash` has incompatible parameters.
  pub fn insert(&mut self, key: usize, minhash: &RMinHash) -> PyResult<()> {
    self.ensure_digest_len(minhash.num_perm())?;
    self.ensure_seed_compatible(minhash.seed())?;
  ...
  pub fn insert_many(
    ...
    for (offset, minhash) in iterator.enumerate() {
      ...
      self.insert(start_key + offset, &minhash)?;
    }
    Ok(())
  }
  ...
  pub fn insert_matrix(
    ...
    self.ensure_seed_compatible(digest_matrix.seed())?;
  ...
  pub fn is_similar(
    ...
    self.ensure_seed_compatible(minhash1.seed())?;
    self.ensure_seed_compatible(minhash2.seed())?;
  ...
  /// Returns the seed required for compatible `RMinHash` digests.
  ///
  /// # Errors
  ///
  /// Returns an error if this index was deserialized from legacy state that
  /// predates seed metadata.
  ...
  fn __getnewargs__(&self) -> PyResult<(f64, usize, usize, u64)> {
    Ok((self.threshold, self.num_perm, self.num_bands, self.seed()?))
  }
}
