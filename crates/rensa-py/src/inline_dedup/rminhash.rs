...
impl RMinHashDeduplicator {
  ...
  pub fn new(
    ...
    let lsh_index = if use_lsh {
      ...
      Some(RMinHashLSH::new(threshold, num_perm, bands, seed)?)
    } else {
      None
    };
  ...
  pub fn clear(&mut self) {
    let lsh_config = self.lsh_index.as_ref().and_then(|lsh| {
      lsh
        .seed()
        .ok()
        .map(|seed| (lsh.get_num_perm(), lsh.get_num_bands(), seed))
    });
    ...
    if let Some((num_perm, num_bands, seed)) = lsh_config {
