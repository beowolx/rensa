...
pub unsafe fn hash_unicode_ptr(
  ...
  // ASCII unicode uses the same bytes as UTF-8.
  if unsafe { ffi::PyUnicode_IS_ASCII(object_ptr) } != 0 {
    // SAFETY: ASCII storage is a 1-byte array of length `char_len`.
    let data = unsafe { ffi::PyUnicode_1BYTE_DATA(object_ptr) };
    let bytes =
      unsafe { std::slice::from_raw_parts(data.cast::<u8>(), char_len) };
  ...
  debug_assert!(utf8_len >= 0);
  let bytes = unsafe {
    std::slice::from_raw_parts(utf8_ptr.cast::<u8>(), utf8_len as usize)
  };
