...
use rensa_core::utils::ratio_usize;
...
impl RMinHash {
  ...
  pub fn new(num_perm: usize, seed: u64) -> PyResult<Self> {
    ...
    let context = Self::build_context(num_perm, seed)?;
  ...
  /// hash into one or more permutation buckets. Deterministic densification for
  /// empty buckets is optional and controlled via `RENSA_RHO_DENSIFY`.
  ///
  /// # Errors
  ///
  /// Returns an error if `num_perm` is zero, the outer input is not iterable,
  ...
  /// # Arguments
  ///
  /// * `items` - A bytes-like object treated as one token, or an iterable of
  ///   `str`/bytes-like tokens. For backward compatibility, a top-level Python
  ///   `str` is still consumed character-by-character.
  ///
  /// # Errors
  ///
