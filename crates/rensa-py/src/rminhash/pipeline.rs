...
use rayon::prelude::*;
use rensa_core::rminhash::{
  apply_token_hashes_to_values, build_permutations, RMinHashContext,
};
use rensa_core::simd::dispatch::PermutationSoA;
use std::sync::mpsc;
use std::sync::Arc;
use std::thread;

#[inline]
fn rayon_parallelism_enabled() -> bool {
...
impl RMinHash {
  ...
  fn compute_digest_chunk(
    ...
    let use_parallel =
      row_count >= config.doc_par_batch_size && rayon_parallelism_enabled();
    if use_parallel {
      data
  ...
  pub(in crate::rminhash) fn build_digest_matrix_from_flat_token_hashes(
    ...
    Python::attach(|py| {
      py.detach(|| {
        let use_parallel =
          rows >= config.doc_par_batch_size && rayon_parallelism_enabled();
        if use_parallel {
  ...
  pub(in crate::rminhash) fn from_matrix(
    ...
    let context = Self::build_context(matrix.num_perm, matrix.seed())?;
  ...
  pub(crate) fn new_compact(num_perm: usize, seed: u64) -> PyResult<Self> {
    ...
    Ok(Self {
      context: Arc::new(RMinHashContext::new(num_perm, seed).map_err(
        |err| {
          PyValueError::new_err(format!(
            "failed to build compact RMinHash context: {err}"
          ))
        },
      )?),
