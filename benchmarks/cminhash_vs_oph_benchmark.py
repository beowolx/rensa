# benchmarks/cminhash_vs_oph_benchmark.py
import time
...
def run_benchmark():
    ...
    # Test with different sketch sizes
    ...
    for i, result in enumerate(results_table):
        if result["speedup"] > 1:  # OptDens becomes faster
            if i > 0:
                crossover = (results_table[i - 1]["sketch_size"], result["sketch_size"])
            else:
