...
Token extraction reads compact ASCII `str` data inline (the common case for tokenized text) instead of round-tripping through `PyUnicode_AsUTF8AndSize`. The parallel batch sketcher recycles its per-chunk extraction buffers through the pipeline instead of reallocating them per chunk, and one-shot LSH deduplication reuses its per-band hash tables and fans band scans out across threads when a Rayon pool is available.
